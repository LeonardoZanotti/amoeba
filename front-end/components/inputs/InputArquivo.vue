<template>
  <div>
    ...
    >
      <v-card>
        <v-toolbar dark color="primary">
          ...
          <v-spacer />
          <v-toolbar-items>
            <v-btn @click="toggleDialog" icon dark>
              <v-icon>close</v-icon>
            </v-btn>
          </v-toolbar-items>
        </v-toolbar>
        ...
        <v-layout :column="$vuetify.breakpoint.xsOnly" class="content">
          <v-flex v-show="sidebarAberta" class="preview-form">
            ...
            <template v-else>
              <v-btn @click="onUploadAtivado" class="mt-4" color="primary">
                Upload
              </v-btn>
            </template>
            <v-container>
              ...
              <v-layout justify-space-around align-center row pt-2>
                <template v-if="modoEditar === false && modoCriar === false">
                  <v-btn @click="onEscolherAtivado" depressed color="primary">
                    Escolher
                  </v-btn>
                  <v-btn
                    @click="iniciarEdicao"
                ...
                <template v-else-if="modoCriar === true">
                  ...
                  <v-btn @click="cancelarCriacao" outline color="grey darken-2">
                    Cancelar
                  </v-btn>
                </template>
                <template v-else>
                  <v-btn @click="salvarEdicao" depressed color="primary">
                    Salvar
                  </v-btn>
                  <v-btn @click="cancelarEdicao" outline color="grey darken-2">
                    Cancelar
                  </v-btn>
                  <v-btn
    ...
    <v-dialog v-model="confirmarRemoverAberto" max-width="320">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn @click="removerArquivo" color="red lighten-1" dark>
            Remover
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <input ref="input" v-show="false" @change="onInputChange" type="file" />
  </div>
</template>
